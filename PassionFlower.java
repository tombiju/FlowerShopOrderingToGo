...
public double accept(FlowerVisitor flowerVisitor, int amount, double userMoney, boolean isOwner){
  ...
  if(amount>this.amount){
    System.out.println("We do not have that many passion flowers in stock!");
    return 0;
  }
    ...
    System.out.println();
    System.out.println("There are now "+getAmount()+" passion flowers in stock.");
  }
  return actualPrice;
}
else{
  ...
  if(userMoney<orderPrice){
    ...
    this.amount+=amount;
    System.out.print("You have bought "+amount+" passion flowers for $");
